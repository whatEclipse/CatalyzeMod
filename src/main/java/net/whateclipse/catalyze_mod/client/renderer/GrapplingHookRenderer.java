...
import net.whateclipse.catalyze_mod.entity.GrapplingHookEntity;
...
public class GrapplingHookRenderer extends EntityRenderer<GrapplingHookEntity> {
    private static final ResourceLocation ROPE_TEXTURE = ResourceLocation.fromNamespaceAndPath(Catalyze_mod.MODID, "textures/entity/rope.png");
    ...
    public void render(@Nonnull GrapplingHookEntity entity, float entityYaw, float partialTicks, @Nonnull PoseStack poseStack, @Nonnull MultiBufferSource buffer, int packedLight) {
        ...
        if (owner != null) {
            ...
            for (float i = 0; i < length; i += 0.5f) {
                ...
                vertexConsumer.addVertex(matrix4f, sx0, sy0 - width, sz0).setColor(255, 255, 255, 255).setUv(0, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight).setNormal(matrix3f, dirX, dirY, dirZ);
                vertexConsumer.addVertex(matrix4f, sx1, sy1 - width, sz1).setColor(255, 255, 255, 255).setUv(1, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight).setNormal(matrix3f, dirX, dirY, dirZ);
                vertexConsumer.addVertex(matrix4f, sx1, sy1 + width, sz1).setColor(255, 255, 255, 255).setUv(1, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight).setNormal(matrix3f, dirX, dirY, dirZ);
                vertexConsumer.addVertex(matrix4f, sx0, sy0 + width, sz0).setColor(255, 255, 255, 255).setUv(0, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight).setNormal(matrix3f, dirX, dirY, dirZ);
            }
